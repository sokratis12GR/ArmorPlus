...
class JEIClientUtils {
    ...
    companion object {
        ...
        fun addDescription(registry: IModRegistry, item: Item, meta: Int, description: String) {
            registry.addDescription(ItemStackUtils.getItemStack(item, meta), description)
        }

        fun addDescription(registry: IModRegistry, item: Item, description: String) {
            registry.addDescription(ItemStackUtils.getItemStack(item), description)
        }

        fun addDescription(registry: IModRegistry, block: Block, meta: Int, description: String) {
            registry.addDescription(ItemStackUtils.getItemStack(block, meta), description)
        }

        @SideOnly(Side.CLIENT)
        fun addDescription(registry: IModRegistry, block: Block, description: String) {
            registry.addDescription(ItemStackUtils.getItemStack(block), description)
        }
    }
}