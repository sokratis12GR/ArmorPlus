...
public class BlockLavaInfuser extends BlockContainer implements IModelHelper {
    ...
    public BlockLavaInfuser(String name, boolean isCharging) {
        super(Material.ROCK);
        this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH));
        this.isInfusing = isCharging;
        this.setRegistryName(name);
        this.setUnlocalizedName(setName(name));
        this.setResistance((float) 10000.0D);
        this.setHardness((float) 2.5D);
        this.setHarvestLevel(BlockBase.ToolType.PICKAXE.getTool(), 1);
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        if (this.isInfusing) {
            ...
            if (rand.nextDouble() < 0.1D) {
                worldIn.playSound((double) pos.getX() + 0.5D, (double) pos.getY(), (double) pos.getZ() + 0.5D, SoundEvents.ENTITY_LIGHTNING_THUNDER, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
            }

            switch (enumfacing) {
                case WEST:
                    worldIn.spawnParticle(EnumParticleTypes.LAVA, d0, d1 + 1, d2 + d4, -0.5D, 0.5D, 0.0D);
                    worldIn.spawnParticle(EnumParticleTypes.PORTAL, d0, d1 + 1, d2 + d4, -0.5D, 0.5D, 0.0D);
                    worldIn.spawnParticle(EnumParticleTypes.FLAME, d0, d1 + 1, d2 + d4, -0.5D, 0.5D, 0.0D);
                    break;
                case EAST:
                    worldIn.spawnParticle(EnumParticleTypes.LAVA, d0, d1 + 1, d2 + d4, 0.5D, 0.5D, 0.0D);
                    worldIn.spawnParticle(EnumParticleTypes.PORTAL, d0, d1 + 1, d2 + d4, 0.5D, 0.5D, 0.0D);
                    ...
                    break;
                case NORTH:
                    worldIn.spawnParticle(EnumParticleTypes.LAVA, d0, d1 + 1, d2 + d4, 0.0D, 0.5D, -0.5D);
                    worldIn.spawnParticle(EnumParticleTypes.PORTAL, d0, d1 + 1, d2 + d4, 0.0D, 0.5D, -0.5D);
                    worldIn.spawnParticle(EnumParticleTypes.FLAME, d0, d1 + 1, d2 + d4, 0.0D, 0.5D, -0.5D);
                    break;
                case SOUTH:
                    worldIn.spawnParticle(EnumParticleTypes.LAVA, d0, d1 + 1, d2 + d4, 0.0D, 0.5D, 0.5D);
                    worldIn.spawnParticle(EnumParticleTypes.PORTAL, d0, d1 + 1, d2 + d4, 0.0D, 0.5D, 0.5D);
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos) {
        ...
        if (active) {
            worldIn.setBlockState(pos, lavaInfuserCharging.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
            worldIn.setBlockState(pos, lavaInfuserCharging.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)), 3);
        } else {
