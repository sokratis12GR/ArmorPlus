...
public class NormalTerrainGenerator {
    ...
    public void setup(World world, Random rand) {
        ...
        ContextOverworld ctx = new ContextOverworld(minLimitPerlinNoise, maxLimitPerlinNoise, mainPerlinNoise, surfaceNoise, noiseGen5, depthNoise, mobSpawnerNoise);
    ...
    public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, IChunkGenerator generator, Biome[] biomes) {
        ...
        int zChunk = x * chunkSize;
        if (!ForgeEventFactory.onReplaceBiomeBlocks(generator, x, z, primer, this.world))
            return;
        this.depthBuffer = this.surfaceNoise.getRegion(this.depthBuffer, xChunk, zChunk, chunkSize, chunkSize, 0.0625D, 0.0625D, 1.0D);

        for (int i = 0; i < chunkSize; ++i) {
            for (int j = 0; j < chunkSize; ++j) {
                Biome biome = biomes[j + i * chunkSize];
                biome.genTerrainBlocks(this.world, this.random, primer, xChunk + i, zChunk + j, this.depthBuffer[j + i * chunkSize]);
            }
        }
    }
