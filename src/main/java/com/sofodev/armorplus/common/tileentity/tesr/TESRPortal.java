...
public class TESRPortal extends TileEntitySpecialRenderer<TilePortal> {
    private static final ResourceLocation END_SKY_TEXTURE = setRL("textures/environment/end_sky.png");
    private static final ResourceLocation END_PORTAL_TEXTURE = setRL("textures/blocks/portal.png");
    ...
    private FloatBuffer getBuffer(float mark, float pos, float lim, float cap) {
        this.buffer.clear();
        this.buffer.put(mark).put(pos).put(lim).put(cap);
        this.buffer.flip();
        return this.buffer;
    }
}