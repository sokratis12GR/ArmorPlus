...
import static com.sofodev.armorplus.common.registry.entities.mobs.dungeon.skeletalking.projectile.Skeleton.*;
...
public class EntityWitherMinion extends EntityFireball implements IThrowableEntity {
    ...
    protected void onImpact(RayTraceResult result) {
        ...
        if (checkPhase(1200, 0)) {
            for (int min = 0; min < warriorBound; min++) {
                if (checkPhase(1200.0F)) {
                    EntityWitherling minionWarrior = create(blockPos);
        ...
        if (checkPhase(800, 0)) {
            for (int min = 0; min < archerBound; min++) {
                EntityWitherling minionArcher = create(blockPos);
        ...
        if (checkPhase(400, 0)) {
            for (int min = 0; min < paladinBound; min++) {
                if (checkPhase(400.0F)) {
                    EntityWitherling minionPaladin = create(blockPos);
    ...
    private EntityWitherling create(BlockPos pos) {
        EntityWitherling minion = new EntityWitherling(this.world);
