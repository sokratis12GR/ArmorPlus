...
public class CraftingRecipeMaker extends RecipeProvider {
    ...
    public void buildSword(Consumer<FinishedRecipe> con, RegistryObject<? extends Item> sword, ItemLike material, ItemLike handle) {
        ...
        build(con, RecipeCategory.COMBAT, Result.build(sword.get(), path + "_swords", path),
    ...
    public void buildBattleAxe(Consumer<FinishedRecipe> con, RegistryObject<? extends Item> battleAxe, ItemLike material, ItemLike handle) {
        ...
        build(con, RecipeCategory.TOOLS, Result.build(battleAxe.get(), path + "_battle_axes", path),
    ...
    public void buildPickaxe(Consumer<FinishedRecipe> con, RegistryObject<? extends Item> battleAxe, ItemLike material, ItemLike handle) {
        ...
        build(con, RecipeCategory.TOOLS, Result.build(battleAxe.get(), path + "_pickaxes", path),
    ...
    public void buildBow(Consumer<FinishedRecipe> con, RegistryObject<? extends Item> bow, ItemLike material) {
        ...
        build(con, RecipeCategory.COMBAT, Result.build(bow.get(), path + "_bows", path).setSuffix("_alt"),
    ...
    public void buildArmorSet(Consumer<FinishedRecipe> con, ItemLike material,
        ...
        this.buildArmorSet(con, Result.build(head, group, path).setSuffix(suffix), Result.build(chest, group, path)
                        .setSuffix(suffix),
                Result.build(legs, group, path).setSuffix(suffix), Result.build(feet, group, path)
    ...
    public void buildSoul(Consumer<FinishedRecipe> con, RegistryObject<Item> bossSoul,
        ...
        this.build(con, RecipeCategory.MISC, Result.build(bossSoul.get(), 1, "ap_souls", "soul"),
    ...
    public void buildStoneBrick(Consumer<FinishedRecipe> con, RegistryObject<Block> bricks, RegistryObject<Block> tower,
        ...
        //shaped
        this.build(con, RecipeCategory.BUILDING_BLOCKS, Result.build(corner.get(), 4, getGroup(pathCorner), pathCorner),
                GridInput.build("   ", "S  ", "SSS", 'S'), bricks.get()
        );
        this.build(con, RecipeCategory.BUILDING_BLOCKS, Result.build(wall.get(), 6, getGroup(pathWall), pathWall),
                GridInput.build("   ", "SSS", "SSS", 'S'), bricks.get()
        );
        this.build(con, RecipeCategory.BUILDING_BLOCKS, Result.build(tower.get(), 8, getGroup(pathTower), pathTower),
                GridInput.build("S S", "SSS", "SSS", 'S'), bricks.get()
        );
        this.build(con, RecipeCategory.BUILDING_BLOCKS, Result.build(stairs.get(), 4, getGroup(pathStairs), pathStairs),
                GridInput.build("S  ", "SS ", "SSS", 'S'), bricks.get()
        );
        this.build(con, RecipeCategory.BUILDING_BLOCKS, Result.build(slab.get(), 6, getGroup(pathSlab), pathSlab),
