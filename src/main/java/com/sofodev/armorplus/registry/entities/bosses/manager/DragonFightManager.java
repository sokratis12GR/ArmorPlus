...
public class DragonFightManager {
    ...
    private final ServerBossInfo bossInfo = (ServerBossInfo) (new ServerBossInfo(new TranslationTextComponent("entity.armorplus.demonic_dragon"), BossInfo.Color.PINK, BossInfo.Overlay.PROGRESS)).setPlayBossMusic(true).setCreateWorldFog(true);
    ...
    public CompoundNBT write() {
        ...
        if (this.dragonUniqueId != null) {
            compoundnbt.putUUID("Dragon", this.dragonUniqueId);
        }
        ...
        if (this.respawnState == null) {
            ...
            if (state == DragonSpawnState.END) {
                ...
                for (ServerPlayerEntity serverplayerentity : this.bossInfo.getPlayers()) {
                    CriteriaTriggers.SUMMONED_ENTITY.trigger(serverplayerentity, demonicDragonEntity);
                }
            } else {
