...
public class StoneBrickSlabBlock extends Block implements IWaterLoggable {
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context) {
        ...
        if (blockstate.isIn(this)) {
            ...
            BlockState nextState = this.getDefaultState().with(TYPE, SlabType.BOTTOM).with(WATERLOGGED, fluidstate.getFluid() == Fluids.WATER);
            Direction direction = context.getFace();
            return direction != Direction.DOWN && (direction == Direction.UP || !(context.getHitVec().y - (double) blockpos.getY() > 0.5D)) ? nextState : nextState.with(TYPE, SlabType.TOP);
        }
    }
